...
def menu(message):
    markup = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    #каждый раз создаются новые кнопки и
    # меняется размер кнопки на целый ряд, и добавляется с помощью markup.add()
    btn1 = telebot.types.KeyboardButton('Кошелек')
    btn2 = telebot.types.KeyboardButton('Перевести')
    btn3 = telebot.types.KeyboardButton('История')
    markup.add(btn1, btn2, btn3)
...
@bot.message_handler(func=lambda message: int(message.from_user.id) == int(config.tg_Admin) and message.text == "Админка")
